...
class MarkovChain:
    ...
    def message_handler(self, m: Message):
        try:
            ...
            #         return
                ...
                # if str(m).startswith(("!ghelp", "!genhelp", "!generatehelp")):
                #     MarkovChain.send_help_message(self)

                # Ignore the message if it is deemed a command
                if MarkovChain.check_if_other_command(self, m):
                    return
                
                # Ignore the message if it contains a link.
                if MarkovChain.check_link(self, m):
                    return
                ...
                if "emotes" in str(m):
                    ...
                    if "emotesv2_" in str(m):
                        return

                    # Replace modified emotes with normal versions, 
                    # as the bot will never have the modified emotes unlocked at the time.
                    for modifier in MarkovChain.extract_modifiers(m):
                        m = str(m).replace(modifier, "")

                # Ignore the message if any word in the sentence is on the ban filter
                if MarkovChain.check_filter(self, m):
                    logger.warning(f"Sentence contained blacklisted word or phrase:\"{m}\"")
                    return
                    ...
                    # db = Database(chan)
                    try:
                        sentences = sent_tokenize(str(m).strip())
    ...
    def generate(self, params: List[str] = None) -> "Tuple[str, bool]":
        ...
        if len(params) > 0:
            if MarkovChain.check_if_other_command(self, params[0]):
                return "Tonto, eu não executo comandos!", False
        ...
        if len(params) > 1:
            ...
            if key == None:
                ...
                if key == None:
                    ...
                    return f"Ainda não aprendi nada sobre \"{params[0]}\".", False
            ...
            if key:
                ...
                return "Ainda estou aprendendo.", False
        
        # Counter to prevent infinite loops (i.e. constantly generating <END> while below the 
        # minimum number of words to generate)
        i = 0
        while MarkovChain.sentence_length(self, sentences) < self.max_sentence_length and i < self.max_sentence_length * 2:
        ...
        if len(params) > 0 and params == sentences[0]:
            return "Eu não aprendi o que fazer com \"" + detokenize(params[-self.key_length:]) + "\" ainda.", False
